import type { StabilityResult, AssertionOptions } from './types';
import { VisualStabilityError } from './types';
 ...
 */
export function assertVisualStability(
  ...
  if (result.cls > clsThreshold) {
    violations.push(
      `CLS ${result.cls.toFixed(4)} exceeds threshold ${clsThreshold}`
    );
  }

  if (customThreshold !== undefined && result.customScore > customThreshold) {
    violations.push(
      `Custom score ${result.customScore.toFixed(4)} exceeds threshold ${customThreshold}`
    );
  }
  ...
  if (violations.length > 0) {
    ...
    throw new VisualStabilityError(message, result, options);
  }
}
